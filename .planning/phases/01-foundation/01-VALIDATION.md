...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| TBD | TBD | TBD | INFRA-01 | — | `next` upgraded; advisories resolved | shell | `npm audit --omit=dev \| grep -E "high\|critical" \|\| echo OK` | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-01 | — | `next` resolves to `^15.5.x` (target 15.5.15) | shell | `npm ls next \| grep -E "next@15\.5\."` | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-02 | — | `next-themes`, `cmdk` installed; zero peer warnings | shell | `npm ls next-themes cmdk 2>&1 \| grep -E "ERR\|peer dep missing\|UNMET" \|\| echo OK` | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-03 | — | CI workflow file present with all 5 steps | shell | `test -f .github/workflows/ci.yml && grep -cE "lint\|typecheck\|test\|knip\|build" .github/workflows/ci.yml \| awk '$1>=5{exit 0}{exit 1}'` | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-03 | — | Knip hard-fails on orphan finding (no `--no-exit-code`) | shell | `grep -c "no-exit-code" .github/workflows/ci.yml` (must be 0) | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-04 | T-INFRA-04 (clickjacking, MIME sniffing, sensor abuse) | Security + engineer headers present on response | shell | `npm run dev &; sleep 3; curl -sI http://localhost:3000/ \| grep -E "Strict-Transport-Security\|X-Content-Type-Options\|Referrer-Policy\|X-Frame-Options\|Permissions-Policy\|x-built-with"; kill %1` | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-05 | T-INFRA-05 (placeholder leak to prod) | postbuild grep fails build on placeholder hit | shell | `cp lib/portfolio-data.ts /tmp/pd.bak; echo "TODO: leak" >> lib/portfolio-data.ts; npm run build; rc=$?; mv /tmp/pd.bak lib/portfolio-data.ts; [ $rc -ne 0 ]` | n/a | ⬜ pending |
| TBD | TBD | TBD | INFRA-05 | — | postbuild grep passes on clean build | shell | `npm run build 2>&1 \| grep -E "INFRA-05.*clean\|✓.*placeholder"` | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-01 | — | `lib/types.ts` exports the 7 expected interfaces | shell | `grep -cE "^export (interface\|type) (Profile\|Project\|Social\|Experience\|Writing\|ShippedApp\|StackCategory)" lib/types.ts` (≥ 7) | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-01 | — | TypeScript strict compiles with new shapes | shell | `npm run typecheck` (exit 0) | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-02 | — | `lib/portfolio-data.ts` exists and exports typed dataset | shell | `test -f lib/portfolio-data.ts && grep -cE "^export const (PROFILE\|PROJECTS\|SOCIALS\|EXPERIENCE\|WRITING\|SHIPPED\|STACK)" lib/portfolio-data.ts` | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-02 | T-DATA-02 (`beck@example.com`, `Product Studio` leak) | Real identity + socials + email; TBD only on bio/projects/writing/shipped/experience | shell | `grep -E "example\.com\|Product Studio" lib/portfolio-data.ts \|\| echo OK`; `grep -E "TODO:" lib/portfolio-data.ts \| wc -l` (must be > 0 to confirm TBD markers used) | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-03 | — | `lib/fallback-data.ts` deleted; no remaining imports | shell | `! test -f lib/fallback-data.ts && ! grep -rE "fallback-data" lib/ app/` | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-04 | — | `lib/api.ts` adapted; `getJson` ISR + silent-fallback preserved | shell | `grep "function getJson\|export async function getJson" lib/api.ts && grep -cE "getProfile\|getProjects\|getSocials\|getExperience\|getWriting\|getShipped\|getStack" lib/api.ts` (≥ 6) | n/a | ⬜ pending |
| TBD | TBD | TBD | DATA-05 | — | `app/components/homepage.tsx` + `homepage.test.tsx` adapted (D-17) — build + test green | shell | `npm test 2>&1 \| grep -E "passed\|✓"`; `npm run typecheck` (exit 0) | n/a | ⬜ pending |
| TBD | TBD | TBD | ROUTE-03 | — | `lib/routes.ts` exports 7-entry typed const; `metadataBase` set | shell | `grep -c "pathname:" lib/routes.ts` (= 7); `grep "metadataBase" app/layout.tsx` | n/a | ⬜ pending |
| TBD | TBD | TBD | ROUTE-03 | — | Build emits zero `metadataBase` warnings | shell | `npm run build 2>&1 \| grep -i "metadataBase" \|\| echo OK` (no warnings) | n/a | ⬜ pending |
| TBD | TBD | TBD | TEST-01 | — | `homepage.test.tsx` adapted (D-17), still green | shell | `npm test` (exit 0; existing test continues to pass) | yes (preserved) | ⬜ pending |
...
- [ ] Sampling continuity: no 3 consecutive tasks without an automated verify (Phase 1 is verify-dense — every task ends in a shell check)
- [ ] Wave 0 covers the homepage adapter dependency (D-17)
- [ ] No watch-mode flags in commands (Vitest in `run` mode only)
- [ ] Feedback latency < 60 s for full suite
