# Requirements Archive: v1.0 Terminal Portfolio MVP
...
- [x] **INFRA-01**: `next` upgraded from `15.3.2` to `^15.5.x` (resolves tracked security advisories surfaced in `.planning/codebase/CONCERNS.md`)
- [x] **INFRA-02**: `next-themes@^0.4.6` and `cmdk@^1.1.1` installed (verified React 19 / Next 15 compatible per STACK.md)
- [x] **INFRA-03**: GitHub Actions CI runs `lint + typecheck + test + knip + build` on every PR (Knip catches orphan files/exports introduced by the migration)
- [x] **INFRA-04**: `next.config.ts` declares custom HTTP headers (e.g. `x-portfolio-source`, `x-built-with`) for engineer-discoverable metadata
- [x] **INFRA-05**: `prebuild` script greps the build output for placeholder strings (`lorem`, `example.com`, `placeholder`, `TODO`) and fails the build on any hit
- [x] **DATA-01**: `lib/types.ts` rewritten to the terminal data model (`Profile`, `Project`, `Social`, `Experience`, `Writing`, `ShippedApp`, `StackCategory`); becomes the single source of truth shared with the sibling backend
- [x] **DATA-02**: `lib/portfolio-data.ts` created with the full typed dataset used as the static fallback (real values, no placeholders)
- [x] **DATA-03**: `lib/fallback-data.ts` and the legacy `lib/types.ts` shape deleted in the same commit that introduces their replacements (no orphan code per Pitfall 11)
- [x] **DATA-04**: `lib/api.ts` adapted to fetch the new shape from `portfolio-services/`; existing `getJson` ISR + graceful-fallback pattern preserved (per STACK.md)
- [x] **DATA-05**: `lib/routes.ts` route registry exports a typed `const` array consumed by `Sidebar`, `CommandPalette`, and `app/sitemap.ts` (single source of truth)

### Shell (SHELL, THEME)

- [x] **SHELL-01**: Persistent terminal shell at `app/(terminal)/layout.tsx` (route group) — never unmounts on navigation between the 7 sibling routes
- [x] **SHELL-02**: `app/layout.tsx` remains a Server Component; only thin client islands (`TopBar`, `Sidebar`, `CommandPalette`, `LiveClock`) carry `"use client"` (per Pitfall 9 — RSC boundary discipline)
- [x] **SHELL-03**: Top bar renders three macOS-style traffic-light dots, the path label `~/portfolio — bakytbek@dev — zsh`, a ⌘K trigger button, a theme toggle, a live HH:MM clock, and a **persistent resume download button visible on every viewport** (per Risk 3 — recruiter usability)
- [x] **SHELL-04**: 240px sidebar renders the EXPLORER file tree from `lib/routes.ts`, the dashed-border "for recruiters" resume card, and the STATUS block (`● Available for hire`, `uptime`, `tz`)
- [x] **SHELL-05**: Active sidebar file is derived from `useSelectedLayoutSegment()` — never mirrored into Context or a store (per ARCHITECTURE.md anti-pattern); active file shows accent text, accent background tint, and 2px left border
- [x] **SHELL-06**: Breadcrumb row renders `~/portfolio / <activeFile>` left and `press ⌘K for commands` hint right; pulled from `usePathname()` in a client component
- [x] **SHELL-07**: Footer renders `© <year> <name>  ·  built with React  v1.0.0` on every view
- [x] **SHELL-08**: 920px max-width main content column with `32px 40px 80px` padding; each view begins with a `$ <command>` prompt line + `slideIn` micro-animation (single 0.25s play)
- [x] **SHELL-09**: Cursor blink (1s `steps(2)` infinite, 8×14px accent block) is the only looping animation in the entire app
- [x] **THEME-01**: `next-themes` `ThemeProvider` mounted in root layout with `attribute="data-theme"`; respects `prefers-color-scheme` on first load when no stored value; default theme = dark (per handoff)
- [x] **THEME-02**: Accent hue managed by an independent inline pre-paint script in `app/layout.tsx` that reads `localStorage["portfolio-accent"]` and sets `document.documentElement.style.setProperty('--accent-hue', hue)` synchronously before paint (per Risk 1 — `next-themes` does NOT manage CSS variables)
- [x] **THEME-03**: `app/globals.css` defines the full oklch token palette derived from `--accent-hue` (matrix=145, amber=75, cyan=200, magenta=340) for both light and dark themes
- [x] **THEME-04**: `@supports (color: oklch(0 0 0))` sRGB fallbacks present on every accent token (per Pitfall 8)
- [x] **THEME-05**: User-facing accent picker UI exposes the four hues (matrix, amber, cyan, magenta); selection persists to `localStorage` and updates `--accent-hue` without a page reload or layout thrash
- [x] **THEME-06**: JetBrains Mono loaded via `next/font/google` (weights 400/500/600/700, `display: 'swap'`, `--font-mono` CSS variable, monospace fallback chain `ui-monospace, "SF Mono", "Cascadia Mono", monospace` with `adjustFontFallback` to keep CLS < 0.1 per Pitfall 10)

### Routing & SEO (ROUTE, SEO)

- [x] **ROUTE-01**: Seven per-view App Router routes exist under the terminal-shell layout: `/` (about), `/projects`, `/stack`, `/experience`, `/writing`, `/contact`, `/shipped`
- [x] **ROUTE-02**: Each `page.tsx` exports a unique `metadata` object (or `generateMetadata`) with route-specific `title`, `description`, and `alternates.canonical` (per Risk 4)
- [x] **ROUTE-03**: `metadataBase: new URL(process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000")` set on root `app/layout.tsx` metadata (resolves the existing `metadataBase` warning surfaced in CONCERNS.md)
- [x] **ROUTE-04**: `app/sitemap.ts` enumerates all 7 routes by mapping over `lib/routes.ts` (no hardcoding); `app/robots.ts` references the sitemap URL; `lastModified` set to build time
- [x] **ROUTE-05**: Terminal-styled 404 page at `app/not-found.tsx` rendering inside the terminal shell (`$ cat /var/log/portfolio.log | grep "$pathname"` style) with links to all 7 views; returns HTTP 404 (verified with `curl -I`)
...
- [x] **SEO-05**: External links use a shared `<ExternalLink>` component that sets `target="_blank"` and `rel="noopener noreferrer"`
- [x] **SEO-06**: Per-project anchors (`/projects#project-name`) — deferred to v1.x per FEATURES.md prioritization (anchors will churn during initial content writing)

### Views (VIEW)

Each view is RSC by default; client behavior lives in shell-level islands.

- [x] **VIEW-01**: `about.md` view at `/` — H1 name (26px/700), `// Sr. Software Engineer` subline in accent, two-paragraph bio (max 68ch), 3 stat cards (`7+ Years shipping`, `AWS Cloud-native`, `AI Agentic systems`) in a 480px row, CTA row with primary `↓ resume.pdf` + ghost social buttons (lowercased + trailing slash)
- [x] **VIEW-02**: `projects/` view at `/projects` — `$ ls -la projects/` prompt, `total N · sorted by year desc` subhead, 3-column grid (`32px 1fr 110px`) per project: index, name+summary+tech chips, year+status+role
- [x] **VIEW-03**: `stack.json` view at `/stack` — `$ cat stack.json | jq` prompt, syntax-highlighted JSON inside `<pre>` card (punctuation muted, keys warn-yellow, string values accent), top-right copy-JSON button
- [x] **VIEW-04**: `experience.log` view at `/experience` — `$ git log --oneline --decorate experience.log` prompt, each row: 7-char hex hash (warn), role (accent/600), `@ company` (muted), period right-aligned (muted/12px), summary below (max 64ch)
- [x] **VIEW-05**: `writing/` view at `/writing` — `$ ls writing/ && cat *.md` prompt, per post: `<DATE UPPERCASED · READTIME>` micro-meta, `› <title>` (16px/600/accent), excerpt (13px/max 64ch); v1 ships zero or N posts (decision in CONTENT-04)
- [x] **VIEW-06**: `contact.sh` view at `/contact` — `$ ./contact.sh --whoami` prompt, lead paragraph, card (max-width 480px) with 90px label column + accent link rows, footer CTAs (resume + `github ↗`); email is a `mailto:` link with copyable plain-text address
- [x] **VIEW-07**: `shipped.app` view at `/shipped` — 7th view (added beyond handoff); per app: name, platforms, real App Store / Play Store deep links, role, year; App Store / Play Store badges as inline SVG; copyable share URLs
- [x] **VIEW-08**: Shared primitives extracted: `app/components/primitives/prompt-line.tsx` (the `$ <command>` line), `tech-chip.tsx`, `kbd.tsx` — used across views; per Pitfall 9 these stay RSC-friendly (no `"use client"`)

### Command Palette (PALETTE)

- [x] **PALETTE-01**: `cmdk` mounted as a single `CommandPalette` client island available globally; opens on ⌘K / Ctrl-K, closes on Esc / backdrop click; modal centered, 520px max-width, mounted at `15vh`
- [x] **PALETTE-02**: Verb taxonomy of **at least 15 verbs** (per FEATURES.md differentiator threshold): open each of the 7 views (7), download resume (1), toggle theme (1), open each social (≥3), copy email (1), copy GitHub profile URL (1), share this view / copy current URL (1), cycle accent hue (1) — total ≥16
- [x] **PALETTE-03**: Type-to-filter against item label and aliases (e.g. `mail` matches `copy email`); result count exposed via `aria-live` region
- [x] **PALETTE-04**: Focus trap inside the modal; on close, focus restored to the element that opened it (per Pitfall 6); palette uses `cmdk`'s built-in `Dialog.Title`
...
- [x] **A11Y-01**: Skip-link to `#main-content` revealed on `:focus` at the top of `<body>` (WCAG 2.4.1)
- [x] **A11Y-02**: All interactive elements (sidebar buttons, palette items, palette trigger, theme toggle, accent picker, links) show a 2px accent `:focus-visible` outline with 2px offset; default outlines never killed without replacement
...
- [x] **A11Y-05**: Semantic landmarks: `<header>` for top bar, `<nav aria-label="File explorer">` for sidebar, `<main id="main-content">` for content, `<footer>` for credits
- [x] **A11Y-06**: Live clock has `aria-hidden="true"` (decoration; no SR announcement every 30s)
...
- [x] **A11Y-08**: Full keyboard nav: tab order top-bar → palette button → theme toggle → sidebar files → main content → footer; ⌘K trap; Esc restores focus
...
- [x] **BACKEND-03**: API contract documented at `portfolio-services/docs/api-contract.md`; frontend and backend type changes ship in paired commits referencing each other (per ARCHITECTURE.md hand-mirror strategy)
- [x] **BACKEND-04**: Sibling backend deployed before frontend cuts over to live API; static fallback in `lib/portfolio-data.ts` covers the gap until backend is live
...
- [x] **TEST-01**: `app/components/homepage.tsx` and `app/components/homepage.test.tsx` deleted in the same commit that introduces their replacement shell skeleton (no orphan code per Pitfall 11)
- [x] **TEST-02**: Vitest coverage for the terminal shell — `TopBar` renders all required affordances, `Sidebar` renders all 7 routes from `lib/routes.ts`, active state derives from URL
- [x] **TEST-03**: Vitest coverage for the command palette — opens on ⌘K, closes on Esc, filters items, restores focus on close
- [x] **TEST-04**: Vitest coverage for theme + accent — toggle persists to localStorage, accent picker updates `--accent-hue` and persists, no SSR flash assertion (mock localStorage to verify pre-paint script)
- [x] **TEST-05**: One smoke test per view (renders, has unique `<title>`, has prompt-line)
...
|-------------|-------|--------|
| INFRA-01 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| INFRA-02 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| INFRA-03 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| INFRA-04 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| INFRA-05 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| DATA-01 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| DATA-02 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| DATA-03 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| DATA-04 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| DATA-05 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-01 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-02 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-03 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-04 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-05 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-06 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-07 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-08 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| SHELL-09 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| THEME-01 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| THEME-02 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| THEME-03 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| THEME-04 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| THEME-05 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| THEME-06 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| ROUTE-01 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| ROUTE-02 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| ROUTE-03 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| ROUTE-04 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| ROUTE-05 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
...
| SEO-05 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-01 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-02 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-03 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-04 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-05 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-06 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-07 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| VIEW-08 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
| PALETTE-01 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| PALETTE-02 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| PALETTE-03 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| PALETTE-04 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| PALETTE-05 | Phase 4 | Complete (04-01 + 04-02) |
| A11Y-01 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| A11Y-02 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| A11Y-03 | Phase 5 | Complete (05-03) |
| A11Y-04 | Phase 2 | Complete |
| A11Y-05 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| A11Y-06 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| A11Y-07 | Phase 5 | Complete (05-07) |
| A11Y-08 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
...
| TEST-01 | Phase 1 | Complete (phase-verified at milestone close 2026-05-14) |
| TEST-02 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| TEST-03 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| TEST-04 | Phase 2 | Complete (phase-verified at milestone close 2026-05-14) |
| TEST-05 | Phase 3 | Complete (phase-verified at milestone close 2026-05-14) |
...
*Last updated: 2026-05-14 — all 89 v1 requirements verified complete at milestone close*
