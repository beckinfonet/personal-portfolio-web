...
Brownfield redesign of an existing Next.js 15 / React 19 portfolio into a high-fidelity terminal/IDE shell with seven per-view App Router routes, light/dark themes, four hue-swappable accents, a âK command palette, full mobile responsiveness, and matching backend shape changes in the sibling `portfolio-services/` repo.

## Milestones

- â **v1.0 Terminal Portfolio MVP** â Phases 1â7 (shipped 2026-05-14, see [milestones/v1.0-ROADMAP.md](milestones/v1.0-ROADMAP.md))
- ð§ **v1.1 GitHub Repo Stats Enrichment** â Phases 8â11 (planned 2026-05-21)

## Phases

<details>
<summary>â v1.0 Terminal Portfolio MVP (Phases 1â7) â SHIPPED 2026-05-14</summary>
...
### ð§ v1.1 GitHub Repo Stats Enrichment (Phases 8â11)
...
**Success Criteria** (what must be TRUE):
  ...
  3. Mobile readability validated at 480px: strip wraps gracefully or truncates the language list to top-1 when width-constrained (LIST-06 â visually confirmed at the 480px breakpoint, no horizontal scroll on the projects card row)
...
- Phase 10 LIST-* and DETAIL-* surfaces share `lib/github.ts` but render independently â parallelizable within the phase (two plans, one per surface, is a reasonable carving)
