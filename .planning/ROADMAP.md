...
- [ ] **Phase 8: Project Schema Extension** — Add optional `repoUrl?: string` to Project across backend + frontend in a paired FE+BE commit
...
**Goal**: Backend and frontend agree on a new optional `Project.repoUrl?: string` field — the contract that every later v1.1 phase compiles against
...
**Success Criteria** (what must be TRUE):
  1. `GET /api/projects` on a re-seeded backend returns 3 projects, each with a `repoUrl` field populated (HTTPS GitHub URL), no `_id` / `__v` leakage — verified by backend Jest spec covering presence/absence shape
  2. `lib/types.ts` `Project` interface exports `repoUrl?: string` with JSDoc documenting its purpose ("Repo URL for GitHub stats fetch; distinct from polymorphic `link` field"); `npm run build` + `tsc --noEmit` clean
  3. `lib/portfolio-data.ts` static fallback byte-mirrors `portfolio-services/src/seed/projects.json` for all three project entries including the new `repoUrl` field (D-14 discipline preserved); FE vitest asserts the field appears on every PROJECTS entry where present
...
**Goal**: `lib/github.ts` module fetches three GitHub REST endpoints, returns typed `GitHubRepoStats | null` with daily ISR cache, token auth, disk-cache fallback, and never throws to callers
**Depends on**: Phase 8
**Requirements**: GH-01, GH-02, GH-03, GH-04, GH-05, GH-06, GH-07, GH-08, GH-09, GH-10
**Success Criteria** (what must be TRUE):
  1. `lib/github.ts` exports `getRepoStats(repoUrl: string): Promise<GitHubRepoStats | null>` and the `GitHubRepoStats` type (shape: `{ createdAt, pushedAt, languages, commitCount }`); module uses native `fetch` only, with zero new prod dependencies (`npm ls @octokit/rest` returns nothing)
...
**Success Criteria** (what must be TRUE):
  1. Every project card on `/projects` that has a `repoUrl` AND non-null stats renders a one-line strip under the card formatted `<commits> commits · <lang1>[ / <lang2>][ / <lang3>] · <duration>` (e.g. `247 commits · TS / CSS · 4mo`), prefixed with a `gh:` (or octocat) source token; cards without `repoUrl` or with null stats render unchanged (no strip, no placeholder, no "private" label)
...
- Phase 8 schema work must complete before Phases 9/10 can compile against the new `repoUrl` field; no in-phase parallelism (single paired commit)
